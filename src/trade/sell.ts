...
import { executeSell } from "../../exchange/executor";
...
export default {
  data: new SlashCommandBuilder()
    .setName("sell")
    .setDescription("Place a market sell order for a trading pair")
    ...
    .addNumberOption(option =>
      option
        .setName("amount")
        .setDescription("Amount of base asset to sell")
        .setRequired(true)
    ),
  ...
  async execute(interaction: CommandInteraction) {
    ...
    // 2️⃣ Execute sell order
    try {
      const order = await executeSell(userId, symbol, amount);
      ...
      return interaction.reply({
        embeds: [Embed.error(`❌ Failed to execute sell order: ${error.message}`)],
        ephemeral: true,
      });
    }
  },
